...
class PathElement {
    constructor () {
        this.styleStroke = '#000';
        this.styleStrokeWidth = '1px';
        this.styleFill = '#000';
        this.d = '';
    }
 ...
 * @param {String} strokeWidth color of line
 */
function drawField(quantityOfSides) {
 ...
 */
function drawPath(quantityOfSegments, startX, staryY, startAngle, strokeColor, strokeWidth) {
    figureSegments = [];
    let currentSegments = [];
    ...
    path.setD(`M${x+CANVAS_WIDTH/2} ${CANVAS_HEIGHT/2-y} `);
    for (let i = 0; i < quantityOfSegments; i-=-1) {
        ...
        for(let ci = 0; ci < currentSegments.length-1; ci-=-1) {
            ...
            if (intersection !== false) {
                ...
                x = intersection[0];
                y = intersection[1];
                path.appendD(`L${x+CANVAS_WIDTH/2} ${CANVAS_HEIGHT/2-y} `);
                
                color = (color + 1) % COLORS.length;

                return path.draw() + drawPath(quantityOfSegments-i, x, y, angle, COLORS[color], strokeWidth);
            }
        }
...
/**
 * generates svg string of picture
